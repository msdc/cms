...
{
    public class TemplateDaoImpl : ITemplateDao
    {
        public DataSet GetTemplateById(string id)
        {
            string sql = "select * from tsys_template where id = @id";
        ...
        {
            string sql = "SELECT * FROM tsys_template ORDER BY ID ASC";
            return SqlHelper.Query(CommandType.Text, sql, null);
        }

        public DataSet GetTemplateId()
        {
            string sql = "select id from [tsys_template]";
            return SqlHelper.Query(CommandType.Text, sql, null);
        }

        public DataSet GetTemplate()
        {
            string sql = "select * from [tsys_template] order by id";
            return SqlHelper.Query(CommandType.Text, sql, null);
        }

        public DataSet GetTemplateInScope(string scopeStr)
        {
            string sql = "select * from [tsys_template] where id in (" + scopeStr + ") order by id";
            return SqlHelper.Query(CommandType.Text, sql, null);
        }

        public int DeleteTemplate(string id)
        {
            string sql = "delete from tsys_template where id = @id";
            SqlParameter[] parm = new SqlParameter[] { new SqlParameter("@id", id) };
        ...
        {
            string sql = "update tsys_template set title = @title, content = @content, Modified = @Modified where id = @id";
        ...
        {
            string sql = "insert into tsys_template(title,content, Created, Modified) values(@title, @content, @Created, @Modified)";
            SqlParameter[] parm = new SqlParameter[] 
            { 
                new SqlParameter("@title", title),
                new SqlParameter("@content", content),
